...
type TaskPrivateData struct {
	// A durable, not-yet-submitted HY request. NOT_START is claimable; SUBMITTED
	// means a worker owns the POST and it must never be replayed automatically.
	GMICloudImageRequest string `json:"gmicloud_image_request,omitempty"`
...
type TaskBillingContext struct {
	DeferredSettlement bool               `json:"deferred_settlement,omitempty"` // Credit only after successful completion.
	EstimatedQuota     int                `json:"estimated_quota,omitempty"`     // Signed cost, separate from Task.Quota's reservation.
...
func (t *Task) HasDeferredBilling() bool {
	return t.PrivateData.BillingContext != nil && t.PrivateData.BillingContext.DeferredSettlement
}
...
func InitTask(platform constant.TaskPlatform, relayInfo *commonRelay.RelayInfo) *Task {
	...
	if relayInfo != nil {
		...
		privateData.ModelMappingFullEnabled = relayInfo.IsModelMappingFullActive()
	}
